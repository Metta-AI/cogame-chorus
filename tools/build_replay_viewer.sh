...
# `coworld build` pre-creates the bundle's parent directory; a fresh CI
# checkout does not, and a hook that resolves its output path against a
# missing parent exits 1 on the first run (ecos, 2026-08-23). Create it
...
test -s "${dist}/chorus_replay.wasm"
test -s "${dist}/chorus_replay.js"

rm -rf "${output_dir}"
mkdir -p "${output_dir}/assets"
cp "${dist}/chorus_replay.js" "${dist}/chorus_replay.wasm" "${output_dir}/"
...
test -f "${output_dir}/chrome_common.js"
grep -q 'data-replay' "${output_dir}/static_replay.js"
echo "chorus replay viewer bundle: ${output_dir}"
