...
# Substitute: chorus, coworld-chorus, 4.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-chorus:ci)
#   SMOKE_SLUG                 game slug                        (chorus)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/chorus)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/chorus-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-chorus:ci}}"
slug="${SMOKE_SLUG:-chorus}"
...
if [ -n "${ANTHROPIC_API_KEY:-}" ]; then
  echo "ANTHROPIC_API_KEY present: prompt player model path enabled"
else
  echo "no ANTHROPIC_API_KEY: prompt players use their scripted fallback"
fi
...
# --------------------------------------------------------------------------
# Every PLAYER container must also exit 0. The template checks only the
# game's exit code, but hosted certification checks the players too: whisky's
# receiveMessage RAISES on a close frame, so a player that does not treat a
# dead socket as a normal end of episode exits 1 and the cert reports
# player_error even though the game itself was fine (raid 0.1.3 -> 0.1.4).
# The race passes one dispatch and fails the next, so it has to be asserted
# here rather than noticed later.
# --------------------------------------------------------------------------
player_failures=0
for ((slot = 0; slot < seats; slot++)); do
  player_exit="$(docker inspect -f '{{.State.ExitCode}}' "${prefix}-p${slot}" 2>/dev/null || echo missing)"
  player_state="$(docker inspect -f '{{.State.Status}}' "${prefix}-p${slot}" 2>/dev/null || echo missing)"
  if [ "${player_state}" = "running" ]; then
    echo "player ${slot}: still running after the game exited; stopping it" >&2
    docker stop -t 10 "${prefix}-p${slot}" >/dev/null 2>&1 || true
    player_exit="$(docker inspect -f '{{.State.ExitCode}}' "${prefix}-p${slot}" 2>/dev/null || echo missing)"
  fi
  if [ "${player_exit}" != "0" ]; then
    echo "FAIL: player container ${slot} exited ${player_exit}" >&2
    player_failures=$((player_failures + 1))
  else
    echo "player ${slot}: exit 0"
  fi
done
