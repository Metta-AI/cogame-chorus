## Chorus static replay viewer, wasm side.
##
## JS hands the raw replay bytes to ch_load_replay; this module parses them
## with the SAME sim code the game server runs, re-derives the per-event
## frames with replayMatch, and exposes the enriched payload (identical shape
## to the game's /replay websocket message) for the shared renderer.js to
## draw.
...
proc chLoadReplay(data: ptr uint8, length: cint): cint
  ...
  try:
    ...
    config.bars = replay["config"]{"bars"}.getInt(8)
    ...
    payload = $ %*{
      "type": "replay",
      "protocol": replay{"protocol"}.getStr("chorus.replay.v1"),
      "names": replay["names"],
...
proc chPayloadPointer(): ptr uint8 {.exportc: "ch_payload_ptr", cdecl.} =
  if payload.len == 0:
    nil
  else:
    cast[ptr uint8](payload[0].addr)

proc chPayloadLength(): cint {.exportc: "ch_payload_len", cdecl.} =
  cint(payload.len)
